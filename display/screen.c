...
static void _putchar_color(char c, vga_color_t fg) {
  switch (c) {
...
void terminal_erase(void) {
  ...
  size_t idx = terminal_row * VGA_WIDTH + terminal_col;
  terminal_buf[idx] =
      vga_entry(TERMINAL_DEFAULT_COLOR_BG, TERMINAL_DEFAULT_COLOR_FG, ' ');
  _update_cursor();
}
