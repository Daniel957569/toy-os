...
static unsigned int _stou(const char **ptr) {
  unsigned int num = 0;
  while (isdigit(**ptr)) {
 ...
 */
static size_t _reverse_and_pad(char *buf, size_t len, unsigned int width,
                               unsigned int flags) {
  char temp[len];
 ...
 * Returns length of the final string.
 */
static size_t _itos(char *buf, unsigned long val, bool negative,
                    unsigned long base, unsigned int width,
                    unsigned int flags) {
  size_t len = 0;
  ...
  if (!(flags & FLAG_LEFTALIGN) && (flags & FLAG_ZEROPAD)) {
    if ((width > 0) &&
        (negative | (flags & FLAG_PLUSSIGN) | (flags & FLAG_SPACESIGN)))
      width--;
 ...
 * Returns length of the final string.
 */
static size_t _ftos(char *buf, double val, bool negative, unsigned int width,
                    unsigned int precision, unsigned int flags) {
  size_t len = 0;
 ...
 */
static void _vprintf(vga_color_t fg, const char *fmt, va_list va) {
  /**
  ...
  while (true) {
    ...
    if (*fmt == '\0') {
      if (seg_end > seg_start) {
        terminal_write_color(seg_start, (size_t)(seg_end - seg_start), fg);
      }
      break;
    }
    ...
    if (*fmt == '%') {
      ...
      case 'x':;
        ...
        size_t i_written = _itos(i_buf, i_val, i_negative, base, width, flags);
      ...
      case 'F':
        ...
        size_t f_written =
            _ftos(f_buf, f_val, f_negative, width, precision, flags);
      ...
      case 'p': /** Pointer type. */
        ...
        size_t p_written = _itos(p_buf, p_val, false, 16, width, flags);
 ...
 */
static void _vsnprintf(char *buf, size_t count, const char *fmt, va_list va) {
